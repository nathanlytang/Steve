...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName("ip").setDescription("Minecraft server IP address"))
    .addIntegerOption((option) => option.setName("port").setDescription("Minecraft server port number"))
    .addStringOption((option) => option.setName("name").setDescription("Minecraft server name"))
    .addStringOption((option) => option.setName("footer").setDescription("Minecraft server footer"))
    .addBooleanOption((option) => option.setName("query").setDescription("Enable or disable query"));
...
export async function execute(options: CommandOptions) {
    ...
    function replyError(arg: string, err: Error | unknown) {
        console.error(
            `\x1b[31m\x1b[1mError setting ${arg} for server ${serverID} (${interaction.guild?.name}):\x1b[0m`
        );
        console.error(err);
        settingsModifiedEmbed.addFields({
            name: `⛔ ${arg.charAt(0).toUpperCase() + arg.slice(1)}`,
            value: `There was an error setting ${arg}!`,
        });
    }
    ...
    const args = {
        ...
        query:
            typeof interaction.options.get("query")?.value === "boolean"
                ? interaction.options.get("query")?.value
                : null,
    };

    if (Object.values(args).filter((p) => p !== null).length === 0) {
        ...
        try {
            if (args.query) {
                ...
                try {
                    ...
                    settingsModifiedEmbed.addFields({
                        name: "Query",
                        value: "Server querying enabled!  Server querying will be used instead of ping.",
                    });
                } catch (err) {
                    replyError("query", err);
                }
            } else {
                // Use ping instead of query
                try {
                    const sql = "UPDATE guild_data SET query = ? WHERE guild_id = ?;";
                    ...
                    settingsModifiedEmbed.addFields({
                        name: "Query",
                        value: "Server querying disabled!  Server pinging will be used instead.",
                    });
                } catch (err) {
                    replyError("query", err);
                }
    ...
    if (args.ip) {
        ...
        if (
            /^(10)\.(.*)\.(.*)\.(.*)$/.test(args.ip) ||
            /^(172)\.(1[6-9]|2[0-9]|3[0-1])\.(.*)\.(.*)$/.test(args.ip) ||
            /^(192)\.(168)\.(.*)\.(.*)$/.test(args.ip)
        ) {
            console.log(
                `\x1b[31m\x1b[1mError setting IP for server ${serverID} (${interaction.guild?.name}):  Private IP address\x1b[0m`
            );
            const privateIPEmbed = new Discord.EmbedBuilder().setTitle("Invalid IP Address").setColor("#E74C3C")
        ...
        try {
            if (args.port < 1 || args.port > 65535) {
                ...
                const badPortEmbed = new Discord.EmbedBuilder()
                    ...
                    .setDescription(
                        `Please ensure your port is a number between 1 and 65535!  No changes have been made.`
                    );
        ...
        try {
            if (args.footer.length > 50) {
                const longFooterEmbed = new Discord.EmbedBuilder()
                    ...
                    .setColor("#E74C3C")
                    .setDescription(
                        `The maximum footer length is 50 characters.  No changes to the footer have been made.`
                    );
                ...
                try {
                    ...
                    if (args.footer.length === 0) {
                        settingsModifiedEmbed.addFields({
                            name: "Footer",
                            value: "Server footer successfully removed!",
                        });
                    } else {
                        settingsModifiedEmbed.addFields({
                            name: "Footer",
                            value: `Server footer set to \`${args.footer}\``,
                        });
                    }
                } catch (err) {
