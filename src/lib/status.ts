    ...
    try {
        if (rows[0].url == "") {
            ...
            return interaction.editReply({
                content: `Your server IP has not been set up!  Please use \`/setup\` to get the setup information.`,
            });
        }

        if (rows[0].query === 1) {
            // Grab server information using query if querying enabled
                        ...
                        try {
                            ...
                            ping.ping(4000, -1, (pingErr: Error, res: any) => {
                                ...
                                try {
                                    ...
                                    console.log(
                                        `Server ${serverID} (${interaction.guild?.name}): Player number does not match list`
                                    );
                                    playerList = "Unknown";
                                }
                                ...
                                const statusEmbed = new Discord.EmbedBuilder()
                                    ...
                                    .addFields(
                                        {
                                            name: "Players",
                                            value: `${stat.numplayers}/${stat.maxplayers}\n`,
                                            inline: true,
                                        },
            ...
            // Grab server information using ping if querying disabled
            try {
                // Send a ping request to the server IP/port to grab server icon
                const ping = new mcping.MinecraftServer(rows[0].url, rows[0].port);
                ping.ping(4000, -1, (pingErr: Error, res: any) => {
